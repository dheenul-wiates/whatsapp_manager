...
export default function AdminLayout({
  ...
  return (
    <div className="h-screen overflow-hidden bg-[#F7F8FA] flex">
      ...
      <aside className="w-60 bg-white border-r border-[#E5E7EB] flex flex-col shrink-0">
        ...
        <div className="h-16 px-5 flex items-center gap-3 border-b border-[#E5E7EB]">
          ...
          <div>
            ...
            <p className="text-xs text-[#6B7280] mt-0.5">Platform Panel</p>
          </div>
        </div>
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-[#6B7280] hover:bg-[#F7F8FA] hover:text-[#111827] transition-colors group"
              >
        ...
        <div className="px-3 py-4 border-t border-[#E5E7EB] shrink-0">
          <form action="/api/admin/logout" method="POST">
            <button
              type="submit"
              id="admin-logout-btn"
              className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-[#6B7280] hover:bg-red-50 hover:text-red-600 transition-colors w-full"
            >
      ...
      {/* Main content */}
      <main className="flex-1 min-w-0 overflow-y-auto pb-10">{children}</main>
    </div>
  )
}
